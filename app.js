...
document.addEventListener('DOMContentLoaded', () => {
  ...
  startBtn.addEventListener('click', () => {
    ...
    // Run the real Python NLP/ML pipeline. The final UI is unlocked only after the API returns.
    state.analysisPromise = analyzeWithBackend(claim);
    ...
    const progressInterval = setInterval(() => {
      ...
      if (currentProgress >= 100) {
        ...
        // Wait for the real NLP/ML result before showing the verified workflow.
        Promise.resolve(state.analysisPromise).then(() => {
